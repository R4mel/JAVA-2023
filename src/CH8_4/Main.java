...
//        System.out.println("Person::clone()");
    ...
    public void writeBinary(DataOutputStream dout) throws IOException {  // 8_4
        ...
        dout.writeInt(getId());
        dout.writeDouble(getWeight());
        dout.writeBoolean(getMarried());
...
//        System.out.println("Student::clone()");
    ...
    public void writeBinary(DataOutputStream dout) throws IOException {  // 8_4
        ...
        dout.writeUTF(getDepartment());
        dout.writeDouble(getGPA());
        dout.writeInt(getYear());
    }
...
//    }
    ...
    public void writeBinary(DataOutputStream dout) throws IOException {  // 8_4
        super.writeBinary(dout);
        dout.writeUTF(getCompany());
        dout.writeUTF(getPosition());
    }
...
class Factory {
    ...
    public static void outputPerson(PrintStream fout, Person t) {
        fout.println(t.getDelimChar() + " " + t);
    }

    public static Person inputPerson(DataInputStream dataIn) throws IOException { // 8_4
...
//        t = 1700000000000L;  // 2023-11-15 오전 07:13; 자동 체크 때 사용할 예정임
    ...
    void saveBinaryFile(String pathName) throws IOException { // 8_4
        var fileOut = new FileOutputStream(pathName);
        var bufOut = new BufferedOutputStream(fileOut);
        var dataOut = new DataOutputStream(bufOut);
    ...
    void loadBinaryFile(String pathName) throws IOException {  // 8_4
        var fileIn = new FileInputStream(pathName);
        var bufIn = new BufferedInputStream(fileIn);
        var dataIn = new DataInputStream(bufIn);

        list.clear();

        while (true) {
            try {
                var p = Factory.inputPerson(dataIn);
                if (p != null) list.add(p);
    ...
    void loadBinaryFrom() throws IOException { // menu item 20: 8_4
        var f = selectFile("binary", "load", true);
        if(f != null) loadBinaryFile(f.getPath());
        else return;
    }
